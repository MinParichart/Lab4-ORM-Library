...
router.get("/books", async (req: Request, res: Response) => { // GET localhost:3010/books
...
router.get("/books/:id", async (req, res) => { // GET localhost:3010/books/1
...
router.get("/members/:id", async (req, res) => { // GET localhost:3010/members/1
  const id = parseInt(req.params.id);
...
router.get("/borrowing-history", async (req: Request, res: Response): Promise<void> => {
...
router.get("/borrowing-history/:id", async (req: Request, res: Response): Promise<void> => { // void เป็นการบอกว่า ไม่มีการ return ค่า
...
router.get("/borrowed-books/:id", async (req, res) => { // localhost:3010/borrowed-books/1
...
// ใหม่
router.post("/books", (req, res) => { // POST localhost:3010/books
  const newBook: Book = req.body;
  addBook(newBook)
    .then((addedBook) => {
      res.json(addedBook);
...
router.post('/upload', upload.single('file'), async (req: any, res: any) => { // POST localhost:3010/upload
    try {
      const file = req.file;
      if (!file) {
